...
import { user, insertUsers } from "../fixtures/user.fixture";
import {
  userOneAccessToken,
  adminAccessToken,
...
describe("Auth routes", () => {
  ...
  describe("POST /v1/auth/refresh-tokens", () => {
    ...
    test("should return 401 error if refresh token is expired", async () => {
      ...
      const refreshToken = tokenService.generateToken(user._id, expires);
      await tokenService.saveToken(
        refreshToken,
        user._id,
        expires,
  ...
  describe("POST /v1/auth/verify-email", () => {
    test("should return 204 and verify the email", async () => {
      ...
      const verifyEmailToken = tokenService.generateToken(user._id, expires);
      await tokenService.saveToken(
        verifyEmailToken,
        user._id,
        expires,
...
describe("Auth middleware", () => {
  ...
  test("should call next with no errors if user does not have required rights but userId is in params", async () => {
    ...
    const req = httpMocks.createRequest({
      headers: { Authorization: `Bearer ${userOneAccessToken}` },
      params: { userId: user._id.toHexString() },
    });
    const next = jest.fn();
